...
public class Connection implements Runnable {
    ...
    public void run() {
        ...
        LOGGER.info("project.Proxy {}:{} <-> {}:{}", clientsocket.getInetAddress().getHostName(), clientsocket.getPort(), serverConnection.getInetAddress().getHostName(), serverConnection.getPort());

        new Thread(new C2SProxy(clientsocket, serverConnection)).start();
        new Thread(new S2CProxy(serverConnection, clientsocket)).start();
        new Thread(() -> {
            while (true) {
